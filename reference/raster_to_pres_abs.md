...
values are set to 1 (presence) and zeros remain 0 (absence).
Additionally, it allows for the conversion of NA values to 0, and/or 0
values to NA, based on the user's choice.

## Usage

``` r
raster_to_pres_abs(raster = NULL, na_to_0 = TRUE, zero_to_na = FALSE)
```
...
- raster:
  ...
  The input raster map. It must be of class `PackedSpatRaster`,
  `RasterLayer`, or `SpatRaster`. This parameter cannot be NULL.

- na_to_0:

  A logical value indicating whether NA values should be converted to 0.
  Defaults to `TRUE`.

- zero_to_na:

  A logical value indicating whether 0 values should be converted to NA.
  Defaults to `FALSE`.

## Value

A raster map where values have been converted according to the specified
parameters. This object is of the same class as the input object.
