...
#'
#' Display one or more HTML strings in the RStudio Viewer pane.
#' Each element of `text` is rendered on a separate line/block while
#' preserving all HTML markup and CSS styling supported by the embedded
#' browser used by RStudio.
#'
#' This function is intended for interactive use inside RStudio only.
#'
#' @param text Character vector containing HTML strings.
#'   Each element is rendered as a separate block.
#' @param title Character scalar specifying the HTML document title.
#' @param css Optional character scalar containing custom CSS.
#'   If `NULL`, a default stylesheet is used.
#' @param background Character scalar specifying the page background
#'   colour.
#' @param viewer_height Numeric scalar passed to
#'   [utils::browseURL()] when supported by the current RStudio version.
#'
#' @details
...
render_html <- function(
    text, title = "HTML Preview", css = NULL,
    background = "white", viewer_height = NULL) {

  # Validation -----------

  if (!rstudioapi::isAvailable()) {
    warning(
      "`render_html()` can only be used from within RStudio.",
      call. = FALSE, immediate. = TRUE)
  }

  if (is.null(text)) {
    ecokit::stop_ctx("`text` must be a non-missing character vector.")
  }

  if (!is.character(text)) {
    ecokit::stop_ctx("`text` must be a character vector.")
  }

  if (length(text) < 1L) {
    ecokit::stop_ctx("`text` must contain at least one element.")
  }

  if (!is.character(title) || length(title) != 1L || is.na(title)) {
    ecokit::stop_ctx("`title` must be a non-missing character scalar.")
  }

  if (!is.null(css) &&
      (!is.character(css) || length(css) != 1L || is.na(css))) {
    ecokit::stop_ctx("`css` must be NULL or a character scalar.")
  }
  ...
  body_html <- paste0(
    "<div class='html-block'>", text, "</div>", collapse = "\n")
  ...
  writeLines(text = html, con = file, useBytes = TRUE)
  ...
  if (is.function(viewer)) {
    if (!is.null(viewer_height)) {
      viewer(file, height = viewer_height)
    } else {
      viewer(file)
    }
  } else {
